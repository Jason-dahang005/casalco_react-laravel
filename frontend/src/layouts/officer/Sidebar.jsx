...
const Sidebar = () => {
  const menus = [
    ...
    { name: "Membership Application", path: "/officer/membership-application", icon: AiOutlineFolderOpen },
              ...
              { menus?.map((menu, i) => (
                <NavLink activeClassName='bg-[#00001a]' to={menu?.path} key={i} className="group flex items-center text-sm gap-3.5 font-medium p-2 hover:bg-[#00001a] rounded-md">
                  ...
                  <h2 
                      ...
                      className={`whitespace-pre duration-500 ${!open && 'opacity-0 translate-x-72 overflow-hidden'}`}>{ menu?.name }</h2>
